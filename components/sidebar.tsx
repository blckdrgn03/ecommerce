...
export default function Sidebar({ isBarOpen, setIsBarOpen }: { isBarOpen: boolean, setIsBarOpen: React.Dispatch<React.SetStateAction<boolean>> }) {
    ...
    return (
        <div onClick={() => isBarOpen && setIsBarOpen(false)} className={`fixed xl:static top-14 sm:top-16 bottom-0 left-0 xl:mt-[74px]  overflow-hidden  w-full  transition-all duration-300 ${isBarOpen ? " backdrop-blur-sm xl:backdrop-blur-none bg-black/[0.25] xl:bg-transparent  dark:xl:bg-transparent dark:bg-white/[0.1]  xl:w-[calc(25%-4px)]" : " xl:w-0"} ${isZUp ? "z-40" : "-z-50"}`}>
