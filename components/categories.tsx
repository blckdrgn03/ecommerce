    ...
    return (
        <section>
            <div  className="flex pt-4 sm:pt-6 md:pt-8 lg:pt-10 justify-between">
                ...
                <Link
                    href="/categories"
                    className="text-neutral-700 dark:text-neutral-400 flex gap-1 items-center hover:text-black dark:hover:text-white transition-all hover:font-semibold"
                >
                    <div>See all</div>
                    <IoIosArrowForward />
                </Link>
            ...
            <div className="relative">
                <button 
                    onClick={scrollPrev}
                    className="absolute p-1 text-3xl md:text-4xl rounded-lg left-1 text-white/[0.25] transition-all top-1/2 hover:scale-y-125 active:scale-y-75 lg:p-2 -translate-y-1/2  hover:text-white/[0.75] z-30"
                > 
                    <IoIosArrowBack />
                </button>
                <div
                    ref={containerRef}
                    id="scroll-container"
                    className="flex gap-2 sm:gap-3 py-2 sm:py-3 overflow-x-scroll no-scrollbar"
                    >
                    {categories.map((category) => (
                        <Link
                            href={"/categories/" + category.name}
                            id="category-item"
                            className={`aspect-[5/5] overflow-hidden shrink-0 flex flex-col justify-end w-[calc(33.3333%-6px)] sm:w-[calc(25%-10px)] lg:w-[calc(20%-10px)] p-2 sm:p-3 relative rounded-md ${
                                !isBarOpen && "xl:w-[calc(16.6666%-10px)]"
                            }`}
                            key={category.id}
                        >
                            <Image
                                className="object-center object-cover bg-blue-500 -z-20"
                                fill
                                alt={category.name}
                                src={category.img}
                            />
                            <div className="text-center text-sm hidden sm:block md:text-md text-neutral-300">
                                {category.products} products
                            </div>
                            <div className="text-center text-lg sm:text-xl capitalize font-bold">
                                {category.name}
                            </div>
                        </Link>
                    ))}
                ...
                <button 
                    onClick={scrollNext} 
                    className="absolute text-white/[0.25] transition-all md:text-4xl top-1/2 hover:scale-y-125 active:scale-y-75 -translate-y-1/2 lg:p-2 right-1 hover:text-white/[0.75] text-3xl rounded-lg p-1 z-30"
                >
                    <IoIosArrowForward />
                </button>
            </div>
