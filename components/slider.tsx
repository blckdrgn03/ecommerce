        ...
        <div className="w-full aspect-[7/5] md:aspect-video xl:aspect-[2/1] rounded-md bg-blue-500 relative flex overflow-hidden">
            {specials.map((special: Special) => {
                return (
                    <div style={{ transform: `translateX(calc(-100% * ${sliderIndex}))` }} key={special.id} className={`shrink-0 w-full aspect-[7/6] sm:aspect-[7/5] md:aspect-video transtion-all duration-500 flex  items-end justify-between gap-1 px-4 py-5 sm:py-7 md:px-6 md:py-9 lg:px-8`}>
                        ...
                        <div className="">
                            <h2><span className="text-black dark:text-white text-lg  sm:text-xl md:text-2xl lg:text-3xl font-semibold">${Math.round(special.price - special.price * special.discount / 100)}</span>&nbsp; <span className="line-through text-xs sm:text-sm md:text-md lg:text-lg text-gray-700 dark:text-gray-300">${special.price}</span><span className="text-xs font-semibold sm:text-sm md:text-md lg:text-lg"> - %{special.discount}</span></h2>
                            <p className=" text-gray-700 dark:text-gray-300 w-[30ch] sm:w-auto truncate sm:whitespace-normal sm:overflow-visible sm:text-clip lg:text-lg text-xs sm:text-sm md:text-md">{special.description}</p>
            ...
            <div className="absolute p-1  rounded-full bottom-1 left-1/2 flex items-end gap-1 -translate-x-1/2">
                {specials.map((special: Special, i: number) => (
                    <button onClick={() => setSliderIndex(i)} className={`h-2 w-2 transition-all duration-500 dark:bg-white/[0.25] bg-black/[0.25] rounded-full dark:hover:bg-gray-300 hover:bg-gray-700 ${sliderIndex == i ? " w-8" : ""}`} key={special.id}></button>
                ))}
                <div
                style={{ transform: `translateX(calc(12px * ${sliderIndex})) translateY(-50%)` }}
                className="absolute dark:bg-gray-300 bg-gray-700 transition-all duration-500 rounded-full left-1 top-1/2  h-2 w-8 "
                ></div>
